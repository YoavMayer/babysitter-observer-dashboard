...
describe('parser', () => {
  ...
  // (AC-30, AC-31, AC-32). Evidence run: DEPUTYsmartSAMAL/
  ...
  describe('breakpoint metadata.payload extraction (UX-R2 §13.1)', () => {
    ...
    const EVIDENCE_QUESTION =
      ...
      'Gidon already pushed: 1655ec3 surgical-writehole (deployed); d9f2231 lockdown (abandoned, excluded)\n\n' +
      'Approve this partition + reconcile plan? (Confirm the boundary is agreed with Gidon.)';
    ...
    const EVIDENCE_PAYLOAD = {
      context: {
        fresh: {
          collision: false,
          note: 'depp-dev unchanged since rebase; no new Gidon pushes into scope',
        },
      },
      expert: 'owner',
      question: EVIDENCE_QUESTION,
      tags: ['approval-gate', 'destructive-git'],
      title: 'Gate 1 — Coordinate partition with Gidon',
    };
    ...
    it('AC-30: parseTaskDetail reads question/title from metadata.payload (evidence shape)', async () => {
      ...
      expect(detail!.breakpoint!.title).toBe('Gate 1 — Coordinate partition with Gidon');
